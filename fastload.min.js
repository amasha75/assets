...
this.loaders.forEach(b=>b.destroy());this.loaders=[];let a=0;for(;5>a++;)if("open"===this.mediaSource.readyState){const b=[];for(let c of this.mediaSource.activeSourceBuffers)b.push(c.updating);if(b.every(c=>!c))return this.mediaSource.endOfStream();await q(20)}this.mediaSource=null}seekTo(a){this.loaders.forEach(b=>b.seekTo(a))}async save(a){const b=await this.getResponse().blob();if(window.navigator.msSaveOrOpenBlob)return navigator.msSaveBlob(b,a);const c=document.createElement("a");c.style.display=
