...
this.config.thread);if(!a.length)return this.dispatcher.isDone();const b=+new Date;if(!this.rtcFound)for(let d of a)if(!d.start)return this.worker.push(this.taskWrap(d)),this.trigger("res.start",d),d.start=b,!1;let c;for(let d of a)if(!d.start){if(!d.rstart){c=d;break}c||(c=d);b-d.rstart>b-c.rstart&&(c=d)}if(c)return this.worker.push(this.taskWrap(c)),this.trigger("res.start",c),c.start=b,!1;this.worker.push(async()=>{await q(200);return{no:-1,data:null,err:null}})}taskFinish(){this.stream&&this.stream.push(this.dispatcher.total,
{done:!0,data:null,err:null,no:1E9})}rtcInit(){clearTimeout(this.rtcLoop);const a=c=>{l.query(this.config.meta,c,(d,e)=>{if(this.stream)return(d=this.stream.item(e))?d.data:d})},b=()=>{const c=this.dispatcher.next(10+this.config.thread);var d=l.getStats();this.trigger("rtc.stat",d,l.id);if(c.length){a:{for(let e in d)if(d[e]&&"open"==d[e].state){d=!0;break a}d=!1}if(d){d=+new Date;let e;for(let f of c)if(!f.rstart){if(!f.start){e=f;break}e||(e=f);d-f.start>d-e.start&&(e=f)}e&&(a(e.no),e.rstart=d,
this.trigger("res.rtc.start",e))}this.rtcLoop=setTimeout(b,(this.worker&&this.worker.pause?5E3:0)+2E3)}else this.rtcLoop=setTimeout(b,2E3)};this.rtcLoop=setTimeout(b,0);this.rtcEvcancel=this.rtcEventInit()}rtcEventInit(){const a=[],b=({id:e,i:f,n:g,uid:h})=>{const [k,m]=e.split("|");this.config.meta==k&&this.trigger("res.rtc.progress",{i:f,n:g,uid:h,no:m})};l.listen("buffer.progress",b);a.push(()=>{l.remove("buffer.progress",b)});const c=({id:e,index:f,buffer:g})=>{g={no:f,data:g,err:null};if(this.stream&&
this.config.meta==e){this.rtcFound++;l.found(this.config.meta,f);e=this.stream.item(f);if(!e||e.err)this.stream.push(f,g),this.trigger("res.rtc.done",g);this.dispatcher.done(f)}};l.listen("data",c);a.push(()=>{l.remove("data",c)});const d=()=>{const e=l.getStats();this.trigger("rtc.stat",e)};l.listen("open",d);a.push(()=>{l.remove("open",d)});l.listen("close",d);a.push(()=>{l.remove("close",d)});l.listen("error",d);a.push(()=>{l.remove("error",d)});return()=>{for(let e of a)e()}}rtcReset(){clearTimeout(this.rtcLoop);
...
class na extends J{constructor(){super(...arguments);this.loaders=[]}async get(a,b,c,d){return await (new J({req:a,start:Number(b),end:Number(c),thread:1,thunk:1048576,meta:a,mirrors:d,nop2p:!0})).start(!1).getResponse().arrayBuffer()}async attach(a,b){const c=new MediaSource;c.addEventListener("sourceopen",async()=>{try{const e=[],f=[],g=[],h=[];for(let k=0;k<b.length;k++){const {req:m,init:u,index:p,mimeCodec:w,len:z,meta:D,mirrors:K}=b[k];var d=z;let oa=/\.webm/.test(m);const [S,L]=await Promise.all([this.get(m,
u.start,u.end+1,K),this.get(m,p.start,p.end+1,K)]);f.push([S,L]);const y=new J({req:m,start:0,end:0,thread:this.config.thread,meta:D,mirrors:K,nop2p:this.config.nop2p});this.loaders.push(y);y.dispatcher=new ma(L,Number(p.end),Number(d),oa);const pa=y.dispatcher.getMap();e.push(pa);const qa=y.start(!0).getResponse().body.getReader();if("open"!==c.readyState)return;const C=new U(a,qa,c,w);y.refBuffer=C;g.push(()=>{y.pause(!1);C.push(S).push(L)});h.push(C.wait());C.listen("error",ra=>{this.trigger("error",
ra);this.pause()});C.listen("pause",()=>{y.pause(!0)});C.listen("start",()=>{y.pause(!1)})}this.trigger("ready",this.loaders,e,f);for(let k of g)k();await Promise.all(h)}catch(e){this.trigger("error",e),this.pause()}});c.addEventListener("sourceclosed",d=>{});c.addEventListener("sourceended",d=>{});this.video=a;this.mediaSource=c;a.src=URL.createObjectURL(c);this.timeUpdate=this.timeUpdate.bind(this);a.addEventListener("timeupdate",this.timeUpdate)}timeUpdate(){if(this.video.buffered.length){const a=
...
this.mediaSource.readyState){const b=[];for(let c of this.mediaSource.activeSourceBuffers)b.push(c.updating);if(b.every(c=>!c))return this.mediaSource.endOfStream();await q(20)}this.mediaSource=null}seekTo(a){this.loaders.forEach(b=>b.seekTo(a))}async save(a){const b=await this.getResponse().blob();if(window.navigator.msSaveOrOpenBlob)return navigator.msSaveBlob(b,a);const c=document.createElement("a");c.style.display="none";c.href=window.URL.createObjectURL(b);c.download=a;document.body.appendChild(c);
