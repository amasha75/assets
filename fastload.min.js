...
b,c,d);return this.parse(a,d)}static async parse(a,b){if(![200,206,304].includes(a.status))throw Error(a.statusText);const c=new Promise((e,f)=>{setTimeout(()=>{f("readtimeout")},b.readtimeout)}),d=new Promise(async(e,f)=>{try{const g=await a.arrayBuffer();e({no:null,data:g,err:null})}catch(g){f(g)}});return Promise.race([c,d])}}class ba{constructor(){}static wrap(a,b,c,d,e,f){return async()=>{let g={no:e,data:null,err:null},h;const k=d-c;let m={timeout:15E3,readtimeout:3E4,cache:!0};for(let u=0;u<
a;u++)try{h=b();g=await aa.fetch(h,c,d,m);g.no=e;g.err=null;const p=g.data?g.data.byteLength:-1;if(p!==k)throw Error(`short read error:expect ${k},got ${p}`);break}catch(p){if(f.item(e))break;m.cache=!1;console.error(p,u,h,c,d,e);g={no:e,err:p,data:null};await q(2E3);m.timeout+=5E3}return g}}}class E{constructor(){this.clientList=new Map}trigger(a,...b){if((a=this.clientList.get(a))&&0!==a.length)for(let c of a)c.apply(null,b)}listen(a,b){const c=this.clientList.get(a);if(!c)return this.clientList.set(a,
...
to:this.id,from:B(),data:b});n(b)}catch(b){r(b)}};this.c.ondatachannel=a=>{if(this.dc)try{this.dc.close()}catch(b){n(b)}this.dc=a.channel;this.dc.binaryType="arraybuffer";this.dcInit();n(a)};this.c.onconnectionstatechange=a=>{n(a)};this.c.onicecandidateerror=a=>{r(a)};this.c.onicegatheringstatechange=a=>{n(a)};this.c.oniceconnectionstatechange=a=>{n(a)};this.c.onicecandidate=a=>{n(a);a.candidate&&(a={event:"candidate",from:B(),to:this.id,data:a.candidate},F().sendJson(a),n("send candidate",a))}}async connect(){n("i connect ",
this.id);const a=()=>{this.init();if(this.dc)try{this.dc.close()}catch(b){n(b)}this.dc=this.c.createDataChannel("dc",{maxPacketLifeTime:2E3});this.dc.binaryType="arraybuffer";this.dcInit()};this.c&&"connected"==this.c.connectionState&&this.dc&&"open"==this.dc.readyState?(G("connection to ",this.id," is already open"),this.send(JSON.stringify({event:"ping"})),clearTimeout(this.restart),this.restart=setTimeout(()=>a(),5E3)):a()}dcInit(){window.addEventListener("beforeunload",()=>{this.c.close();this.dc.close()});
this.dc.onopen=a=>{r("dc open me : "+B()+" remote: "+this.id,a);this.onmsg("open",a)};this.dc.onclose=a=>{r("dc close "+this.id,a);this.onmsg("close",a)};this.dc.onerror=a=>{r("dc error "+this.id,a);this.onmsg("error",a)};this.dc.addEventListener("closing",a=>{r("dc closing "+this.id,a);this.onmsg("closing",a)});this.dc.onmessage=async a=>{clearTimeout(this.restart);let b=a.data;b instanceof Blob&&(b=await a.data.arrayBuffer());this.rx=b instanceof ArrayBuffer?this.rx+b.byteLength:this.rx+b.length;
...
new V(c,Number(d),Number(e)));this.stream=new W(null);this.worker=new X(b,f,g=>this.taskDone(g),()=>{this.taskFinish()});a||(this.init(),this.initial=!0);this.worker.pause=a;return this}pause(a){this.worker&&(this.worker.pause=a);a||this.initial||(this.init(),this.initial=!0)}init(){const {thread:a}=this.config;let b=0;for(;b<a;){const c=this.dispatcher.next(10+a);if(c.length)for(let d of c)if(!d.start){this.worker.push(this.taskWrap(d));this.trigger("res.start",d);d.start=+new Date;break}b++}this.config.nop2p||
!window.RTCPeerConnection||this.rtcLoop||this.config.meta==this.config.req||(this.rtcLoop=setTimeout(()=>this.rtcInit(),1E3))}destroy(){this.remove("");this.pause(!0);this.stream.destroy();this.stream=null;this.rtcReset();this.refBuffer&&(this.refBuffer.q.clear(),this.refBuffer=null)}seekTo(a){this.pause(!1);const b=this.dispatcher.getMap(),c=this.dispatcher.total;for(let d=0;d<c;d++)if(b[d].begin>=a){a=Math.max(d-1,0);this.dispatcher.seekTo(a);this.cachefill(a);break}}cachefill(a){const b=this.dispatcher.total;
let c=!1;for(;a<b;a++){const d=this.stream.item(a);if(!d||d.err||!d.data)break;c||(this.refBuffer.q.clear(),c=!0);this.refBuffer.repush(d.data)}}taskWrap(a){const {m:b,n:c,no:d}=a;let e=0;const f=[];return ba.wrap(this.config.retry,()=>{e++;const g=[this.config.req].concat(this.config.mirrors);let h=g[d%g.length];if(1>=e)return f.push(h),h;h=this.getBestURL(g,f);f.push(h);return h},b,c,d,this.stream)}getBestURL(a,b){const c=a.filter(d=>!b.includes(d));return c.length?c[Math.floor(Math.random()*c.length)]:
a[Math.floor(Math.random()*a.length)]}async taskDone(a){if(!this.stream)return!0;if(-1===a.no)return this.triggerNextTask();const b=this.stream.item(a.no);b||(this.stream.push(a.no,a),this.trigger("res.done",a));this.dispatcher.done(a.no);!this.config.nop2p&&window.RTCPeerConnection&&this.config.meta!=this.config.req&&(b&&!b.err||a.data&&!a.err)&&l.found(this.config.meta,a.no);return!b&&a.err?(this.err=a.err,!0):this.err?!0:this.triggerNextTask()}async triggerNextTask(){const a=this.dispatcher.next(10+
this.config.thread);if(!a.length){if(this.dispatcher.isDone())return!0;this.worker.push(async()=>{await q(200);return{no:-1,data:null,err:null}});return!1}const b=+new Date;if(!this.rtcFound)for(let d of a)if(!d.start)return this.worker.push(this.taskWrap(d)),this.trigger("res.start",d),d.start=b,!1;let c;for(let d of a)if(!d.start){if(!d.rstart){c=d;break}c||(c=d);b-d.rstart>b-c.rstart&&(c=d)}if(c)return this.worker.push(this.taskWrap(c)),this.trigger("res.start",c),c.start=b,!1;this.worker.push(async()=>
{await q(200);return{no:-1,data:null,err:null}})}taskFinish(){this.stream&&this.stream.push(this.dispatcher.total,{done:!0,data:null,err:null,no:1E9})}rtcInit(){clearTimeout(this.rtcLoop);const a=c=>{l.query(this.config.meta,c,(d,e)=>{if(this.stream)return(d=this.stream.item(e))?d.data:d})},b=()=>{const c=this.dispatcher.next(10+this.config.thread);var d=l.getStats();this.trigger("rtc.stat",d,l.id);if(c.length){a:{for(let e in d)if(d[e]&&"open"==d[e].state){d=!0;break a}d=!1}if(d){d=+new Date;let e;
for(let f of c)if(!f.rstart){if(!f.start){e=f;break}e||(e=f);d-f.start>d-e.start&&(e=f)}e&&(a(e.no),e.rstart=d,this.trigger("res.rtc.start",e))}this.rtcLoop=setTimeout(b,(this.worker&&this.worker.pause?5E3:0)+2E3)}else this.rtcLoop=setTimeout(b,2E3)};this.rtcLoop=setTimeout(b,0);this.rtcEvcancel=this.rtcEventInit()}rtcEventInit(){const a=[],b=({id:e,i:f,n:g,uid:h})=>{const [k,m]=e.split("|");this.config.meta==k&&this.trigger("res.rtc.progress",{i:f,n:g,uid:h,no:m})};l.listen("buffer.progress",b);
a.push(()=>{l.remove("buffer.progress",b)});const c=({id:e,index:f,buffer:g})=>{g={no:f,data:g,err:null};if(this.stream&&this.config.meta==e){this.rtcFound++;l.found(this.config.meta,f);e=this.stream.item(f);if(!e||e.err)this.stream.push(f,g),this.trigger("res.rtc.done",g);this.dispatcher.done(f)}};l.listen("data",c);a.push(()=>{l.remove("data",c)});const d=()=>{const e=l.getStats();this.trigger("rtc.stat",e)};l.listen("open",d);a.push(()=>{l.remove("open",d)});l.listen("close",d);a.push(()=>{l.remove("close",
...
c+=8,k=Number(b.getBigUint64(c)),c+=8);k+=a+1;c+=2;a=b.getUint16(c);c+=2;const m=[];let u=h,p=k;for(let w=0;w<a;w++){const z=b.getUint32(c);c+=4;const D=b.getUint32(c);c+=4;c+=4;m.push({reference_type:0,reference_size:z,subsegment_duration:D,durationSec:D/g,startTimeSec:u/g,startRange:p,endRange:p+z-1});p+=z;u+=D}return{version:e,flag:d,referenceId:f,timeScale:g,earliest_presentation_time:h,first_offset:k,reference_count:a,references:m}}}class Q{static info(a){return this.types[a]?this.types[a]:["",
...
for(b=0;b<a.reference_count;b++)d=a.references[b],e=b,c[e]={m:d.startRange,n:d.endRange+1,no:e,begin:d.startTimeSec};this.taskMap=a=c}this.total=Object.keys(this.taskMap).length}next(a=10){const b=[];a=Math.min(this.index+a,this.total);let c=!0,d=this.index;for(let e=this.index;e<a;e++){const f=this.taskMap[e];f.done?c&&(d=f.no+1):(c=!1,b.push(f))}this.index=d;return b}isDone(){for(let a=0;a<this.total;a++)if(!this.taskMap[a].done)return!1;return!0}getMap(){return this.taskMap}seekTo(a){if(a>=this.total)throw Error("index error");
this.index=a}done(a){this.taskMap[a]?this.taskMap[a].done=!0:console.error("error in done")}}class na extends J{constructor(){super(...arguments);this.loaders=[]}async get(a,b,c,d){return await (new J({req:a,start:Number(b),end:Number(c),thread:1,thunk:1048576,meta:a,mirrors:d,nop2p:!0})).start(!1).getResponse().arrayBuffer()}async attach(a,b){const c=new MediaSource;c.addEventListener("sourceopen",async()=>{try{const e=[],f=[],g=[],h=[];for(let k=0;k<b.length;k++){const {req:m,init:u,index:p,mimeCodec:w,
len:z,meta:D,mirrors:K}=b[k];var d=z;let oa=/\.webm/.test(m);const [S,L]=await Promise.all([this.get(m,u.start,u.end+1,K),this.get(m,p.start,p.end+1,K)]);f.push([S,L]);const y=new J({req:m,start:0,end:0,thread:this.config.thread,meta:D,mirrors:K,nop2p:this.config.nop2p});this.loaders.push(y);y.dispatcher=new ma(L,Number(p.end),Number(d),oa);const pa=y.dispatcher.getMap();e.push(pa);const qa=y.start(!0).getResponse().body.getReader();if("open"!==c.readyState)return;const C=new U(a,qa,c,w);y.refBuffer=
C;g.push(()=>{y.pause(!1);C.push(S).push(L)});h.push(C.wait());C.listen("error",ra=>{this.trigger("error",ra);this.pause()});C.listen("pause",()=>{y.pause(!0)});C.listen("start",()=>{y.pause(!1)})}this.trigger("ready",this.loaders,e,f);for(let k of g)k();await Promise.all(h)}catch(e){this.trigger("error",e),this.pause()}});c.addEventListener("sourceclosed",d=>{});c.addEventListener("sourceended",d=>{});this.video=a;this.mediaSource=c;a.src=URL.createObjectURL(c);this.timeUpdate=this.timeUpdate.bind(this);
...
this.loaders.forEach(b=>b.destroy());this.loaders=[];let a=0;for(;5>a++;)if("open"===this.mediaSource.readyState){const b=[];for(let c of this.mediaSource.activeSourceBuffers)b.push(c.updating);if(b.every(c=>!c))return this.mediaSource.endOfStream();await q(20)}this.mediaSource=null}seekTo(a){this.loaders.forEach(b=>b.seekTo(a))}}return na});
