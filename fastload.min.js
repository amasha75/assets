...
b,c,d){this.threadNum=a;this.retry=b;this.callback=c;this.finish=d;this.t=0;this.tasks=[];this.pause=!1}async thread(a){for(a=!1;;)if(this.pause)await t(100);else{var b=this.get();if(b)b=await this.do(b),await this.taskOneDone(b)&&(a=!0);else if(await t(100),a)break}this.taskDone()}push(a){this.tasks.push(a);this.t<this.threadNum&&(this.thread(this.t),this.t++)}get(){return this.tasks.pop()}async taskOneDone(a){return await this.callback(a)}taskDone(){this.t--;0>=this.t&&this.finish()}async do(a){const b=
...
break}catch(p){if(f.item(e))break;m.cache=!1;console.error(p,u,h,c,d,e);g={no:e,err:p,data:null};await t(2E3)}return g}}}class E{constructor(){this.clientList=new Map}trigger(a,...b){if((a=this.clientList.get(a))&&0!==a.length)for(let c of a)c.apply(null,b)}listen(a,b){const c=this.clientList.get(a);if(!c)return this.clientList.set(a,[b]),this;for(a=c.length-1;0<=a;a--)if(c[a]===b)return this;c.push(b);return this}remove(a,b){if(!a)return this.clientList.clear(),this;a=this.clientList.get(a);if(!a)return this;
...
{maxPacketLifeTime:2E3});this.dc.binaryType="arraybuffer";this.dcInit()};this.c&&"connected"==this.c.connectionState&&this.dc&&"open"==this.dc.readyState?(G("connection to ",this.id," is already open"),this.send(JSON.stringify({event:"ping"})),clearTimeout(this.restart),this.restart=setTimeout(()=>a(),5E3)):a()}dcInit(){window.addEventListener("beforeunload",()=>{this.c.close();this.dc.close()});this.dc.onopen=a=>{q("dc open me : "+B()+" remote: "+this.id,a);this.onmsg("open",a)};this.dc.onclose=
a=>{q("dc close "+this.id,a);this.onmsg("close",a)};this.dc.onerror=a=>{q("dc error "+this.id,a);this.onmsg("error",a)};this.dc.addEventListener("closing",a=>{q("dc closing "+this.id,a);this.onmsg("closing",a)});this.dc.onmessage=async a=>{clearTimeout(this.restart);let b=a.data;b instanceof Blob&&(b=await a.data.arrayBuffer());this.rx=b instanceof ArrayBuffer?this.rx+b.byteLength:this.rx+b.length;this.onmsg("message",{data:b})}}async onOffer(a){"closed"==this.c.signalingState?q("onOffer error signalingState is closed"):
...
b){if(a=r.get(a))a.onAnswer(b);else console.error("onAnswer peer not found error")}async onCandidate(a,b){if(a=r.get(a))a.onCandidate(b);else console.error("onCandidate peer not found error")}sendTo(a,b){const c=r.get(a);if(c)return c.send(b);console.error("uuid "+a+" not connected")}broadcast(a){r.forEach(b=>{b.send(a)})}getPeers(){return r.keys()}getStats(){const a={};r.forEach(b=>{a[b.id]=b.stat()});return a}}class ia extends E{constructor(a){super();this.servers=a;this.buffers=new Map;this.m=
...
{id:e,index:f,buffer:d})})}}const P={iceServers:[{urls:"stun:119.29.1.39:3478"},{urls:"turn:119.29.1.39:3478",username:"su",credential:"su"}]};let l;class J extends A{constructor(a){super();this.defaultOpts={retry:5,thread:2,thunk:524288};this.rtcEvcancel=()=>{};this.rtcFound=0;this.config=Object.assign({},this.defaultOpts,a);this.config.nop2p||l||!window.RTCPeerConnection||(l=new O(P),l.init())}start(a){const {thread:b,thunk:c,start:d,end:e,retry:f}=this.config;this.dispatcher||(this.dispatcher=
new V(c,Number(d),Number(e)));this.stream=new W(null);this.worker=new X(b,f,k=>this.taskDone(k),()=>{this.taskFinish()});this.worker.pause=a;let g=0;for(;g<b;){var h=this.dispatcher.next(1);h.length&&(h=h[0],this.worker.push(this.taskWrap(h)),h.start=+new Date);g++}this.config.nop2p||!window.RTCPeerConnection||this.rtcLoop||!1===a||this.config.meta==this.config.req||(this.rtcLoop=setTimeout(()=>this.rtcInit(),1E3));return this}pause(a){this.worker&&(this.worker.pause=a)}destroy(){this.remove("");
this.pause(!0);this.stream.destroy();this.stream=null;this.rtcReset();this.refBuffer&&(this.refBuffer.q.clear(),this.refBuffer=null)}seekTo(a){this.pause(!1);const b=this.dispatcher.getMap(),c=this.dispatcher.total;for(let d=0;d<c;d++)if(b[d].begin>=a){a=Math.max(d-1,0);this.dispatcher.seekTo(a);this.cachefill(a);break}}cachefill(a){const b=this.dispatcher.total;let c=!1;for(;a<b;a++){const d=this.stream.item(a);if(!d||d.err||!d.data)break;c||(this.refBuffer.q.clear(),c=!0);this.refBuffer.repush(d.data)}}taskWrap(a){const {m:b,
n:c,no:d}=a;let e=0;const f=[];return ba.wrap(this.config.retry,()=>{e++;const g=[this.config.req].concat(this.config.mirrors);let h=g[d%g.length];if(1>=e)return f.push(h),h;h=this.getBestURL(g,f);f.push(h);return h},b,c,d,this.stream)}getBestURL(a,b){const c=a.filter(d=>!b.includes(d));return c.length?c[Math.floor(Math.random()*c.length)]:a[Math.floor(Math.random()*a.length)]}async taskDone(a){if(!this.stream)return!0;var b=this.stream.item(a.no);b||(this.stream.push(a.no,a),this.trigger("res.done",
a));this.dispatcher.done(a.no);!this.config.nop2p&&window.RTCPeerConnection&&this.config.meta!=this.config.req&&(b&&!b.err||a.data&&!a.err)&&l.found(this.config.meta,a.no);if(!b&&a.err)return this.err=a.err,!0;if(this.err)return!0;a=this.dispatcher.next(10+this.config.thread);if(!a.length)return!0;b=+new Date;if(!this.rtcFound)for(let d of a)if(!d.start)return this.worker.push(this.taskWrap(d)),this.trigger("res.start",d),d.start=b,!1;let c;for(let d of a)if(!d.start){if(!d.rstart){c=d;break}c||(c=
d);b-d.rstart>b-c.rstart&&(c=d)}c&&(this.worker.push(this.taskWrap(c)),this.trigger("res.start",c),c.start=b)}taskFinish(){this.stream&&this.stream.push(this.dispatcher.total,{done:!0,data:null,err:null,no:1E9})}rtcInit(){clearTimeout(this.rtcLoop);const a=c=>{l.query(this.config.meta,c,(d,e)=>{if(this.stream)return(d=this.stream.item(e))?d.data:d})};a(0);const b=()=>{const c=this.dispatcher.next(10+this.config.thread);var d=l.getStats();this.trigger("rtc.stat",d,l.id);if(c.length){a:{for(let e in d)if(d[e]&&
"open"==d[e].state){d=!0;break a}d=!1}if(d){d=+new Date;let e;for(let f of c)if(!f.rstart){if(!f.start){e=f;break}e||(e=f);d-f.start>d-e.start&&(e=f)}e&&(a(e.no),e.rstart=d,this.trigger("res.rtc.start",e))}this.rtcLoop=setTimeout(b,(this.worker&&this.worker.pause?5E3:0)+2E3)}else this.rtcLoop=setTimeout(b,2E3)};this.rtcLoop=setTimeout(b,2E3);this.rtcEvcancel=this.rtcEventInit()}rtcEventInit(){const a=[],b=({id:e,i:f,n:g,uid:h})=>{const [k,m]=e.split("|");this.config.meta==k&&this.trigger("res.rtc.progress",
{i:f,n:g,uid:h,no:m})};l.listen("buffer.progress",b);a.push(()=>{l.remove("buffer.progress",b)});const c=({id:e,index:f,buffer:g})=>{g={no:f,data:g,err:null};if(this.stream&&this.config.meta==e){this.rtcFound++;l.found(this.config.meta,f);e=this.stream.item(f);if(!e||e.err)this.stream.push(f,g),this.trigger("res.rtc.done",g);this.dispatcher.done(f)}};l.listen("data",c);a.push(()=>{l.remove("data",c)});const d=()=>{const e=l.getStats();this.trigger("rtc.stat",e)};l.listen("open",d);a.push(()=>{l.remove("open",
d)});l.listen("close",d);a.push(()=>{l.remove("close",d)});l.listen("error",d);a.push(()=>{l.remove("error",d)});return()=>{for(let e of a)e()}}rtcReset(){clearTimeout(this.rtcLoop);this.rtcEvcancel();l&&l.clear()}getResponse(){return this.stream.getResponse()}static rtc(){l||(l=new O(P),l.init());return l}}class ja{constructor(a){this.data=a}parse(a=0){const b=this.data;let c=8;var d=b.getUint32(c);const e=d>>>24;d&=16777215;c+=4;const f=b.getUint32(c);c+=4;const g=b.getUint32(c);c+=4;let h,k;0==
e?(h=b.getUint32(c),c+=4,k=b.getUint32(c),c+=4):(h=Number(b.getBigUint64(c)),c+=8,k=Number(b.getBigUint64(c)),c+=8);k+=a+1;c+=2;a=b.getUint16(c);c+=2;const m=[];let u=h,p=k;for(let w=0;w<a;w++){const z=b.getUint32(c);c+=4;const D=b.getUint32(c);c+=4;c+=4;m.push({reference_type:0,reference_size:z,subsegment_duration:D,durationSec:D/g,startTimeSec:u/g,startRange:p,endRange:p+z-1});p+=z;u+=D}return{version:e,flag:d,referenceId:f,timeScale:g,earliest_presentation_time:h,first_offset:k,reference_count:a,
...
this.taskMap[a].done=!0:console.error("error in done")}}class na extends J{constructor(){super(...arguments);this.loaders=[]}async get(a,b,c,d){return await (new J({req:a,start:Number(b),end:Number(c),thread:1,thunk:1048576,meta:a,mirrors:d,nop2p:!0})).start(!1).getResponse().arrayBuffer()}async attach(a,b){const c=new MediaSource;c.addEventListener("sourceopen",async()=>{try{const e=[],f=[],g=[],h=[];for(let k=0;k<b.length;k++){const {req:m,init:u,index:p,mimeCodec:w,len:z,meta:D,mirrors:K}=b[k];
var d=z;let oa=/\.webm/.test(m);const [S,L]=await Promise.all([this.get(m,u.start,u.end+1,K),this.get(m,p.start,p.end+1,K)]);f.push([S,L]);const y=new J({req:m,start:0,end:0,thread:this.config.thread,meta:D,mirrors:K,nop2p:this.config.nop2p});this.loaders.push(y);y.dispatcher=new ma(L,Number(p.end),Number(d),oa);const pa=y.dispatcher.getMap();e.push(pa);const qa=y.start(!0).getResponse().body.getReader();if("open"!==c.readyState)return;const C=new U(a,qa,c,w);y.refBuffer=C;g.push(()=>{y.pause(!1);
